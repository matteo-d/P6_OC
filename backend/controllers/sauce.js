...
exports.createSauce = (req, res, next) => {
  ...
  if (req.body.sauce !== undefined && middlewareSauce.isValidSauceSchema(JSON.parse(req.body.sauce)) ) {
    User.exists(
...
if ( middlewareSauce.isValidSauceSchema ) {
      ...
      Sauce.findOne({ _id: req.params.id })
        .then((sauce) => {
          ...
          if (
            ...
            res
              .status(400)
              .json({ message: "Aucun dislike ou like a annulé " });
          }
        })
        .catch((error) => {
          res.status(400).json({
