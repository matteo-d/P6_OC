...
module.exports = (req, res, next) => {
  try {
    ...
    const decodedToken = jwt.verify(token,'RANDOM_TOKEN_SECRET'); // On vérifier le token avec la clef secrete que l'on a créer dans controllers user
    const userId = decodedToken.userId; // On extrait le userId de l'objet  créer par decodedToken
    if (req.body.userId && req.body.userId !== userId) { // on vérifie que l'User Id  correspond bien au token d'utilisateur 
